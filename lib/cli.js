...
Interface.prototype.run = function () {
    ...
    if (options.reload) {
        if (cluster.isMaster) {
            Server = proton.ReloadingServer;
        }
        else {
            Server = proton.ReloadingChildServer;
        }
    }
    //else if (options.nofork) {
        Server = proton.SingleProcessServer;
    //}
    //else {
    //    Server = proton.MutipleProcessServer;
    //}

    (new Server(options)).run().then(onStart, function (message) {
