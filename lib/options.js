...
var Processor = exports.Processor = function (params) {
    ...
    for (var i = 0, l = options.length; i < l; i++) {
        ...
        if (options[i].alias) {
            if (options[i].alias in this._byAlias) {
                throw new Error('duplicate alias: -' + options[i].alias);
            }
...
Processor.prototype._processOption = function (args, option, name, dir) {
    ...
    if (! validator.test(result)) {
        throw new UserError('value for option ' + name + ' is not a valid ' + option.value);
    }
    return result;
};
...
Processor.prototype._processAliases = function (arg, args, result, dir) {
    for (var i = 1, ilen = arg.length; i < ilen; i++) {
        ...
        if (alias in this._byAlias) {
            result[this._byAlias[alias].name] = this._processOption(
                i === (ilen - 1) ? args : [],
                this._byAlias[alias],
                '-' + alias,
                dir
...
Processor.prototype._helpText = function () {
    ...
    for (var i = 0, ilen = this._options.length; i < ilen; i++) {
        text += '  --' + this._options[i].name +
                ...
                (
                    ('default' in this._options[i]) ?
                        ' (default ' + this._options[i].default +
                        (this._options[i].defaultDescription ? ' - ' + this._options[i].defaultDescription : '') +
                        ')' :
                        ''
                ) +
                '\n';
...
Processor.prototype._processArgs = function (args, result, dir) {
    ...
    for (var i in this._options) {
        if (this._options[i].value === 'path' && result[this._options[i].name]) {
            result[this._options[i].name] = makeAbsolutePath(dir, result[this._options[i].name]);
        }
    }
