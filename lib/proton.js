...
exports.WORKER_HTTP_REQUEST = 'PROTON_WORKER_HTTP_REQUEST';

exports.runWorker = decorateError(function (options, onError) {
    ...
    server = http.createServer(new jsgi.Listener(function (request) {
        ...
        return prepare(WebApp).then(decorateError(function () {
            var response = webapp.handle(request);
            // signal back to the master process that we served a request.
            (process.send && process.send(exports.WORKER_HTTP_REQUEST));
            return response;
        }, onError));
